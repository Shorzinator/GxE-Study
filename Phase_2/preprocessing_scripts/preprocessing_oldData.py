...
def preprocessing_pipeline(features, target, file_path_to_save):
    ...
    X_train = standard_scaling_continuous_variables_old(X_train, feature_cols, target)

    X_test = apply_yeojohnson_transformation(X_test)
    X_test = standard_scaling_continuous_variables_old(X_test, feature_cols, target)

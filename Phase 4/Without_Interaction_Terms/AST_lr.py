...
param_space = {
    ...
    'classifier__solver': ['newton-cg', 'lbfgs', 'saga'], # removed 'sag'
    'classifier__max_iter': [100, 300, 500, 1000], # increased upper limit
...
def run_model_for_outcome(outcome, results_df, features, df):
    ...
    for train_index, test_index in stratified_split.split(features, targets):
        ...
        pipeline = ImbPipeline([
            ...
            ('classifier', LogisticRegression(random_state=0))  # Using Logistic Regression
        ])
        ...
        cv_results_df.to_csv(f"results_LogisticRegression_{outcome}.csv")
        ...
        joblib.dump(best_pipeline, f"best_model_LogisticRegression_{outcome}.joblib")
...
results_df.to_csv(os.path.join(output_dir, "combined_results_RandomizedSearch_SMOTEEN_StratifiedKFold_lr.csv"))

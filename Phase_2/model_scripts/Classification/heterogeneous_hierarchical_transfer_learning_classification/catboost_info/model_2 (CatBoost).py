...
def main(target_variable, race_column="Race", tune_base=False, tune_interim=False):
    ...
    #                                      f"intermediate_model_wRace_woPGS.pkl", "wb"))
    # pd.DataFrame(
    #     {
    ...
    #          f"intermediate_model_metrics_with_tl.csv", index=False)
    ...
    for (race, model_name), model in final_models.items():
        pickle.dump(model, open(
            f"../../../results/models/classification/HetHieTL/{tag}/{model_name}/{model_name}_race_{race}_woPGS.pkl", "wb"))
        pd.DataFrame({
            'Race': [race],
            'Model': [model_name],

...
# Define the hyperparameter space for RandomizedSearchCV for Random Forest
param_space = {
...
def run_model_for_outcome(outcome, results_df, features, df):
    ...
    for train_index, test_index in stratified_split.split(features, targets):
        ...
        pipeline = ImbPipeline([
            ...
            ('classifier', RandomForestClassifier(random_state=0))  # Using Random Forest
        ])
        ...
        cv_results_df.to_csv(f"results_RandomForest_{outcome}.csv")
        ...
        joblib.dump(best_pipeline, f"best_model_rf_{outcome}.joblib")
...
results_df.to_csv(os.path.join(output_dir, "combined_results_RandomizedSearch_SMOTEEN_StratifiedKFold_rf.csv"))

...
# Define the hyperparameter space for RandomizedSearchCV for LGBM
param_space = {
    ...
    'classifier__min_child_samples': [5, 10, 20, 30, 40],
...
def run_model_for_outcome(outcome, results_df, features, df):
    ...
    for train_index, test_index in stratified_split.split(features, targets):
        ...
        pipeline = ImbPipeline([
            ...
            ('classifier', lgb.LGBMClassifier(objective="binary", random_state=0))  # <--- Using LightGBM
        ])
        ...
        cv_results_df.to_csv(f"results_LGBM_{outcome}.csv")
        ...
        joblib.dump(best_pipeline, f"best_model_LGBM_{outcome}.joblib")
...
results_df.to_csv(os.path.join(output_dir, "combined_results_RandomizedSearch_SMOTEEN_StratifiedKFold_lgbm.csv"))
